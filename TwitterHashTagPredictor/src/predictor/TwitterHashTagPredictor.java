...
public class TwitterHashTagPredictor {
	 ...
	 * @param k - the number of HashTaags to recommend, up to k. -1 means get all
	 * @return the k best predicted HashTags
	 */
	public List<HashTagPrediction> predictTopKHashTagsForTweet(String tweet, int k){
		...
		for(TaggedWord tw : taggedWords){
			...
			currentPrediction.confidence = predictionModel.calculateConfidence(tw);
		}
		
		Collections.sort(hashTagPredictions, Collections.reverseOrder());
		//truncate the list to the max size
		return hashTagPredictions.subList(0, Math.min(k, hashTagPredictions.size()));
	}
}
